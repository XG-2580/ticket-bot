...
module.exports = {
	...
	run: async (client, message, args) => {
		if(message.channel.name.includes('ticket-')) {
			...
			if(message.member.hasPermission('ADMINISTRATOR') || message.channel.name === `ticket-${message.author}`) {
				message.channel.messages.fetch().then(async (messages) => {
					...
					const embed = new MessageEmbed()
						...
						.setColor('GREEN');
					member.send('Here is the transcript fo your ticker after you talk with our staff. ', embed);
				}).then(() => {
					try {
