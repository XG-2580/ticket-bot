module.exports = {
	...
	run: async (client, message, args, prefix) => {
		if(message.guild.channels.cache.find(channel => channel.name === `ticket-${message.author}`)) {
		...
		message.guild.channels.create(`ticket-${message.author}`, {
			...
			if(logchannel) {
				logchannel.send(`Ticket ${message.author} created. Click the following to veiw <#${channel.id}>`);
			}
		});
	},
};